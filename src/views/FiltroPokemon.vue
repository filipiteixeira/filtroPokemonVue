<template>
  <div>
    <div class="filtro-geracoes">
      <button @click="changeGeracaoPokemon(1)">Geração 1</button>
      <button @click="changeGeracaoPokemon(2)">Geração 2</button>
    </div>
    ...
    <div class="lista-pokemon" :class="{ fadeInAnimation: filtroPokemon }">
      <div
        class="card-pokemon"
  ...
  methods: {
    async fetchData() {
      ...
      try {
        for (
          this.pokemonInicial;
          this.pokemonInicial <= this.pokemonFinal;
          this.pokemonInicial++
        ) {
          const response = await axios.get(
            `https://pokeapi.co/api/v2/pokemon/${this.pokemonInicial}`
          );
    ...
    changeGeracaoPokemon(geracao: number) {
      if (geracao === 1) {
        this.pokemonInicial = 1;
        this.pokemonFinal = 151;
      } else if (geracao === 2) {
        this.pokemonInicial = 152;
        this.pokemonFinal = 251;
      }
      this.fetchData();
    },
  },
